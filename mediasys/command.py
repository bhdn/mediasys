...
class MediasysCommand(object):

    descr = "The base mediasys command."
    usage = "[options] [args]"
    ...
    def setup_logging(self, config, opts, args):
        ...
        # configparser
        format = config.mediasys.log_format.replace("$", "%")
    ...
    def main(self):
        try:
            ...
            mediasys = MediasysFacade(config)
            self.config = config
            self.opts = opts
            self.args = args
            self.mediasys = mediasys
            self.run()
        except Error, e:
